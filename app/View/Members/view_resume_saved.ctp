...
if(isset($status) && count($status) > 0)
{
    ...
    {
        if($item['RecruitmentStatus']['id'] == $members_employers['MemberEmployer']['recruitment_status_id'])
        {
        ...
        $html_status = $html_status .
            '<li>
            <a ' . $style . ' href="javascript:void(0)" class="tooltip-info change-status" data-rel="tooltip" title="" data-original-title="View" data-status_id="' . $item['RecruitmentStatus']['id'] . '" data-member_recruitment="' . $members_employers['MemberEmployer']['id'] . '">
